...
When building new software, developers usually deal with %a large number of information sources. In such a context, the problem is not the lack but instead 
an overload of information %coming 
from heterogeneous sources. %The proliferation of open source repositories in recent years necessitates suitable toolings to mine the rich data sources~\cite{DBLP:journals/ese/RoccoRSNR21}. %When dealing with a growing information overload when discerning alternative development solutions. 
Recommender systems for software engineering (RSSEs)~\cite{DBLP:books/sp/rsse2014} are an effective means %have been developed % assist software engineers in Thus, recommender systems in software engineering
to provide developers with %instant support %recommendations 
%consisting of 
different items, \eg code~\cite{Fowkes:2016:PPA:2950290.2950319,Nguyen:2019:FRS:3339505.3339636}, third-party components~\cite{NGUYEN2019110460,Ouni:2017:SSL:3032135.3032325}. %, documentation, to name a few.
Nevertheless, while the main effort has been spent to make RSSEs more effective and efficient, there are issues %related to fairness and robustness 
that have not attracted enough attention from the community. %, as shown below.
...
%has shown 
%In particular, even though RSSEs are becoming more and more effective in suggesting handy recommendations, they tend to suffer from popularity bias, \ie favoring items that are relevant mainly because several developers are using them. %While this rewards artifacts that  are likely more reliable and well-documented, it would also mean that missing artifacts are rarely used because they are very specific or more recent.
Through literature reviews, we investigated state-of-the-art studies emerging from %published in 
several SE venues, and found out that RSSEs %while RSSEs have become more effective at suggesting relevant items, they 
...
%So far, there are no concrete countermeasures that can be instantly deployed to defend RSSEs against attacks. 
%, as well as to . %, as well as conceiving counteractions. %
%While the Software Engineering community either underestimates or ignores it, the possibility of using falsified data to trick RSSE is always present, leaving a potential danger to software systems.
%In this respect, we see an urgent need to thoroughly study the related issues, %likely threats, 
...
The core objective %of this proposal 
is to develop effective techniques to improve %address a key challenge, \ie improving 
fairness in RSSEs. %recommender systems for software engineering. %and \emph{(ii)} fortifying RSSEs against adversarial attacks. 
%If my proposal gets funded, I will be able to promote and strengthen collaborations with different research groups in Hanoi. %would it pave the way for the collaboration with School of Information and Communications Technology (SOICT). %research 
%team at the University of L'Aquila. 
%Being Principal Investigator (PI), 
If this proposal receives funding, it will pave the way for the formation of a dedicated team. % at the University of L'Aquila. 
Being Principal Investigator (PI), I will lead the team to work diligently to advance ongoing research efforts, and propose practical solutions that address unresolved issues. % in the field of Machine Learning for Software Engineering. 
